    ...
    <div class="receipt-box">
        ...
        <table class="info-table">
            <tr>
                ...
                <td class="info-value">: {{ $order->resellerInvoice->invoice_number ?? $order->order_number }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="info-value">: {{ $order->payment_type->label() }}</td>
            </tr>
            <tr>
                <td class="info-label">Status Pembayaran</td>
                <td class="info-value">: {{ $order->payment ? $order->payment->payment_status->label() : 'Menunggu Pembayaran' }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="info-value">: {{ $order->order_status->label() }}</td>
            </tr>
        </table>
        ...
        <div style="width: 100%; overflow: hidden; margin-top: 10px;">
            <div class="summary-block">
                <div class="summary-row">
                    ...
                    <div class="summary-value">Rp {{ number_format($order->subtotal_amount, 0, ',', '.') }}</div>
                </div>
                <div class="summary-row">
                    <div class="summary-label">Ongkir</div>
                    <div class="summary-value">Rp {{ number_format($order->shipping_cost, 0, ',', '.') }}</div>
                </div>
                ...
                <div class="summary-row" style="margin-top: 6px;">
                    ...
                    <div class="summary-value">
                        Rp {{ number_format($order->payment && $order->payment->isPaid() ? $order->payment->amount : 0, 0, ',', '.') }}
                    </div>
                </div>
            </div>
        </div>
        ...
        @else
            <div class="status-badge status-unpaid">
                Sisa Tagihan : Rp {{ number_format($order->total_amount - ($order->payment && $order->payment->isPaid() ? $order->payment->amount : 0), 0, ',', '.') }}
            </div>
        @endif
